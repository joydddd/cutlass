...
@step(tag="coord")
def load_a_tile(gA: cute.Tensor, tv_layout: cute.Layout, coord: cute.Coord) -> cute.Tensor:
    """
...
def store_a_tile(reg: cute.Tensor, gO: cute.Tensor, tv_layout: cute.Layout, coord: cute.Coord) -> None:
    """
...
@cute.jit
def cute_add(A: cute.Tensor, O: cute.Tensor):
    ...
    gO = cute.zipped_divide(O, tiler=tv_tiler)
...
def jit_cute_add(A: torch.Tensor) -> torch.Tensor:
    ...
    cuteO = from_dlpack(O, assumed_align=16)


    with Context():
        compiled_func = cute.compile(cute_add, cuteA, cuteO)

        print("Step Registry: ", Context.current().step_registry)
        print("Step Origin Registry: ", Context.current().step_origin_registry)
